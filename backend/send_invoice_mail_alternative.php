<?php
// Alternative email sending script that doesn't require wkhtmltopdf
...
try {
    ...
    error_log("Alternative email - Received input: " . json_encode($input));
    ...
    error_log("Alternative email - Processing email for: $user_email, Booking: $booking_id, Service: $service_name");
    ...
    $html_invoice = generateHTMLInvoice($user_name, $booking_id, $service_name, $total_amount, $service_date, $service_time, $address, $cart_items, $discount, $taxes, $payment_status);
    ...
    <body>
        ...
        <div class='content'>
            ...
            <p>Thank you for choosing NPC Services! Your booking has been confirmed and your invoice is attached below.</p>
    ...
    // Log email attempt
    error_log("Alternative email - Attempting to send HTML email to: $user_email");
    error_log("Alternative email - Subject: $subject");
    
    // Send email
    $mail_sent = mail($user_email, $subject, $message, implode("\r\n", $headers));
    
    // Log the result
    error_log("Alternative email - Mail send result: " . ($mail_sent ? "SUCCESS" : "FAILED"));
    ...
    if ($mail_sent) {
        ...
        try {
            ...
            error_log("Alternative email - Database log result: " . ($log_result ? "SUCCESS" : "FAILED"));
        } catch (Exception $db_error) {
            error_log("Alternative email - Database logging error: " . $db_error->getMessage());
        }
        ...
        error_log("Alternative email - Mail function error: " . json_encode($error));
    ...
    error_log("Alternative email - Send invoice email error: " . $e->getMessage());
    echo json_encode(['status' => 'error', 'message' => 'Internal server error']);
}

// Function to generate HTML invoice
function generateHTMLInvoice($user_name, $booking_id, $service_name, $total_amount, $service_date, $service_time, $address, $cart_items, $discount, $taxes, $payment_status) {
